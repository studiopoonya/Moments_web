...
import { whyUsApi, type WhyUsItemT } from "@/lib/api";

const EMPTY = { icon: "Sparkles", title: "", text: "" };

export function AdminWhyUs() {
  const [items, setItems] = useState<WhyUsItemT[]>([]);
  ...
  async function load() {
    setLoading(true);
    try {
      setItems(await whyUsApi.list(getAdminToken()!));
    } finally {
      setLoading(false);
    }
  }
  ...
  function openEdit(item: WhyUsItemT) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (editingId) {
        await whyUsApi.update(token, editingId, data);
      } else {
        data.append("sort_order", String(items.length));
        await whyUsApi.create(token, data);
      }
      setFormOpen(false);
      await load();
    } catch {
  ...
  async function handleDelete(id: number) {
    const token = getAdminToken();
    if (!token || !confirm("Hapus item ini?")) return;
    await whyUsApi.remove(token, id);
    await load();
  }

  return (
    <AdminLayout title="Kenapa Pilih Kami" subtitle="Kelola alasan & ikon di section 'Kenapa Pilih Kami'">
      <div className="flex justify-end">
        <Button variant="brand" className="rounded-full" onClick={openCreate}>
          <Plus className="size-4" /> Tambah Alasan
        </Button>
      </div>
      ...
      {formOpen ? (
        <form onSubmit={handleSubmit} className="card-soft mt-4 space-y-4 p-6">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-lg text-foreground">{editingId ? "Edit Alasan" : "Alasan Baru"}</h2>
      ...
      <div className="card-soft mt-6 overflow-hidden">
        {loading ? (
          ...
          <p className="p-8 text-center text-sm text-muted-foreground">Belum ada alasan.</p>
        ) : (
          items.map((item) => {
